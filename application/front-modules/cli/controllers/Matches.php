...
class Matches extends CI_Controller {
    ...
    */
    ...
    public function create($what = NULL, $name = NULL, $module = false)
    {
    ...
    public function create_module($app = null, $args = null)
    {

        if(isset($app) && isset($args))
        {
            $this->create_controller_module($app, $args);
            $this->create_view_module($app, $args);
            $this->create_model($app);
        }
        else
        {
            echo $this->_ret.'You need to provide a name for the module';
        }
    ...
    */
    ...
    public function create_controller_module()
    {
        $available = array('extend'=>'extend','e'=>'extend','module'=>'module');
        $params = func_get_args();
        $arguments = array();
        ...
        if(isset($controller))
        {
            ...
            $file_name = $names['file'];
            $directories = $names['directories'];
            $module_location = array_key_exists('module', $arguments) ? $arguments['module'].'/' : $this->_c_module;
            $module_location = str_replace('.', '/', $module_location);

            if(file_exists(APPPATH.$module_location.'controllers/'.$file_name.'.php'))
            {
                echo $this->_ret.$class_name.' Controller already exists in the '.$module_location.'/controllers directory.';
            }
            else
            {
                $f = $this->_get_template('controller');
                ...
                if(!file_exists(APPPATH.$module_location.'controllers/'))
                {
                    mkdir(APPPATH.$module_location.'controllers/', 0777, true);
                }
                if(write_file(APPPATH.$module_location.'controllers/'.$file_name.'.php',$f))
                {
                    echo $this->_ret.'Controller '.$class_name.' has been created inside '.APPPATH.$module_location.'controllers/.';
                    return TRUE;
                }
                else
                {
    ...
    */
    ...
    public function create_view_module($view = NULL)
    {
        $available = array('module'=>'module');
        $params = func_get_args();
        $arguments = array();
        foreach($params as $parameter)
        {
        ...
        if(isset($view))
        {
            ...
            if(file_exists(APPPATH.$module_location.'views/'.$file_name.'.php'))
            {
                echo $this->_ret.$file_name.' View already exists in the '.$module_location.'/views/ directory.';
            }
            else
            {
                $f = $this->_get_template('view');
                ...
                $this->_find_replace['{{VIEW}}'] = $file_name.'.php';
                $f = strtr($f,$this->_find_replace);
                if(!file_exists(APPPATH.$module_location.'views/'))
                {
                    mkdir(APPPATH.$module_location.'views/', 0777, true);
                }
                if(write_file(APPPATH.$module_location.'views/'.$file_name.'.php',$f))
                {
                    echo $this->_ret.'View '.$file_name.' has been created inside '.APPPATH.$module_location.'views/.';
                    return TRUE;
                }
                else
                {
